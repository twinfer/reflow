...
- **`cmd/reflow-cluster`** — operator CLI: offline PKI (`init-ca`, `issue-cert`, `issue-operator`) and mTLS-authenticated cluster ops (`add-node`, `remove-node`, `nodes list`, `partitions list`, `snapshot create/list`) against the Admin gRPC port. `--admin` may point at any cluster node — mutating commands follow `LeaderHint` status details (attached by the server's `requireLeader`) to redirect to the metadata leader. The redirect helper lives at `pkg/reflow/admin.CallWithLeaderRedirect`.
