...
- **`cmd/reflowd`** — the production binary with three subcommand groups:
  ...
  - `reflowd cluster {add-node|remove-node|nodes list|partitions list|snapshot create|list|delete|register-deployment|eventsources|webhooks|apply|export|get|init-kek|create-secret|delete-secret|list-secrets|decrypt-secret|upsert-webhook|transfer-lp|list-lp-transfers}` — mTLS-authenticated RPCs against the admin Connect port, split internally between two services: `reflow.clusterctl.v1.ClusterCtl` (membership, partitions, snapshots, LP transfers) and `reflow.config.v1.Config` (deployments, event sources, webhooks, secrets). Single listener hosts both. `--admin` may point at any cluster node — mutating commands follow `LeaderHint` connect.Error details (attached by each server's `requireLeader`) to redirect to the metadata leader. The redirect helper lives at `pkg/reflowclient.CallWithLeaderRedirect`. Step-2 CLI will split into `reflowd cluster ...` / `reflowd config ...`; step 1 keeps the kitchen-sink `cluster` subcommand wired to both services. Naming mirrors Restate (`cluster-ctrl` = cluster admin; `admin` is the developer/app-config surface), with `admin` flipped to `config` to avoid the overloaded word.
