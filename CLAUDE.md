...
- **`cmd/reflowd`** — the production binary with three subcommand groups:
  ...
  - `reflowd config {register-deployment|eventsources {list|delete}|webhooks {list|delete}|apply|export|get|init-kek|create-secret|delete-secret|list-secrets|decrypt-secret|upsert-webhook}` — same admin listener, dispatched to the `reflow.config.v1.Config` service (app config: deployments, event sources, webhooks, secrets).
