...
- **`internal/ingress/webhook`** — cluster-managed inbound vendor webhook ingress. Records live in shard 0's `WebhookSourceTable`; per-node `Manager` instances reconcile against a `TableNotifier` wake (5s ticker backstop) and atomically swap a fresh path→source snapshot (`atomic.Pointer[map[string]*resolvedSource]`). One stable subtree route at `/webhooks/` (stdlib `http.NewServeMux` trailing-slash semantics) — the handler reads `r.URL.Path` and looks up the live snapshot. Built-in `Verifier`s for Stripe (`Stripe-Signature: t=…,v1=…`, 5min replay window), GitHub (`X-Hub-Signature-256`), and Slack (`v0:ts:body`, 5min replay window) register at package `init()` time via `pkg/webhook.RegisterVerifier`; operators register custom verifiers from `main` before `reflow.Run`. Verified facts (`webhook_vendor`, `stripe_signed_timestamp`, `github_delivery`, …) flow into `SubmitInvocationRequest.metadata` → `ctx.Metadata()`. **Secrets never traverse Raft**: `WebhookSourceRecord.secret_ref` carries one of `env_var_name` / `file_path` / `remote_encrypted { blob_uri, kek_uri }`. The per-node Reconciler resolves on each pass via `os.Getenv`, `os.ReadFile`, or `gocloud.dev/blob.ReadAll` + Tink `registry.GetKMSClient(kek_uri).GetAEAD(kek_uri).Decrypt(ct, []byte(name))` (AAD = webhook name, so a leaked ciphertext for row A can't be replayed onto row B). Hot-rotation: `file_path` via atomic file replacement; `remote_encrypted` via blob overwrite (fleet-wide on next reconcile); `env_var_name` requires a restart. Read/decrypt failure preserves the previously-resolved bytes so a transient FS/KMS hiccup doesn't knock the source offline. Reflow ships `pkg/tinkkms/blob` (URI prefix `blobkms+<gocloud-blob-uri>`, AES-256-GCM with the 32-byte master key fetched from any `gocloud.dev/blob` backend) as the no-managed-KMS fallback, registered once at startup via `sync.Once`; operators register cloud KMS (`tink-go/v2/integration/{gcpkms,awskms,…}`) from `main` before `reflow.Run`. `kek_uri` is dispatched by Tink's process-global registry by URI prefix. Tink's `monitoring.Client` interface is exported but `RegisterMonitoringClient` lives in `tink-go/v2/internal/internalregistry` (blocked from external import in v2.6), so the reconciler resolve path is hand-instrumented via `reflow_webhook_kms_decrypt_total{kek_scheme}` / `_errors_total{name,kek_scheme,stage}` / `_seconds`. The starter authz policy enumerates `/webhooks/*`, `/webhooks/*/*`, `/webhooks/*/*/*` anonymously because the signature *is* the auth — 4+ segment paths require a custom policy since `path.Match` doesn't support `**`. Bootstrap koanf path (`cfg.Webhooks.Sources`) acts as a seed: rejects inline plaintext (would leak via snapshots) and requires `secret_env`, `secret_file`, or `secret_blob_uri`+`secret_kek_uri`. Operators manage post-bootstrap via `reflowd cluster webhooks list / delete` and `cluster apply -f <file>` (`kind: WebhookSource`). See `durable-execution-go-sad.md` §6.14 for the full data flow.
...
- **Cluster-managed app config is on shard 0.** Event sources, webhook sources (and future kinds) live in shard 0's typed tables (`EventSourceTable`, `WebhookSourceTable`). Mutations propose `Command_Upsert*/Delete*` with `Envelope.precondition.if_table_revision_eq` for CAS; the FSM signals CAS failure via `Result.Value = ResultValueFailedPrecondition` (returning an error halts the shard, per `internal/engine/CLAUDE.md`). Per-node subsystems subscribe to a `cluster.TableNotifier` and pull a fresh snapshot on wake (5s ticker backstop). Bootstrap koanf config seeds with `if_table_revision_eq=0` so a racing operator-Apply at the same moment reproducibly conflicts. **Webhook secrets do not traverse Raft**: `WebhookSourceRecord.secret_ref` is a oneof of `env_var_name`, `file_path`, or `remote_encrypted { blob_uri, kek_uri }`. Each node resolves locally at reconcile time; for `remote_encrypted`, ciphertext fetched via `gocloud.dev/blob`, KEK dispatched through Tink's `KMSClient` registry, AAD = webhook name. `pkg/tinkkms/blob` is the shipped fallback; operators import `tink-go/v2/integration/{gcpkms,awskms,…}` from `main` for cloud KMS.
