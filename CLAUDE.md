...
- **`internal/ingress/webhook`** — config-driven inbound vendor webhook ingress. Built-in `Verifier`s for Stripe (`Stripe-Signature: t=…,v1=…`, 5min replay window), GitHub (`X-Hub-Signature-256`), and Slack (`v0:ts:body`, 5min replay window) register at package `init()` time via `pkg/webhook.RegisterVerifier`. The `Manager` mounts one `connectserver.Route` per configured `cfg.Webhooks.Sources` entry on the existing ingress listener (via `ExtraRoutes`), verifies signatures, and dispatches `SubmitInvocation` directly into the in-process `ingress.Server`. Verified facts (`webhook_vendor`, `stripe_signed_timestamp`, `github_delivery`, …) flow into `SubmitInvocationRequest.metadata` → `ctx.Metadata()` so handlers branch on origin without re-verifying. Operators with bespoke vendor schemes register custom verifiers via `pkg/webhook.RegisterVerifier` from their `main` before `reflow.Run` — same escape hatch as `eventsource.RegisterFactory`. The starter authz policy allows `/webhooks/*` anonymously because the signature *is* the auth; tighten via `cfg.Auth.PolicyFile` if you want per-vendor IP allowlists or operator gates on management webhooks. See `durable-execution-go-sad.md` §6.14 for the full data flow and the doctrinal reasoning behind shipping 3 verifiers built-in vs. operator-writes-everything.
