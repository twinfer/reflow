...
- **`cmd/reflwd`** — the production binary with three subcommand groups:
  ...
  - `reflwd config {register-deployment|list-deployments|describe-deployment|delete-deployment|init-kek|create-secret|delete-secret|list-secrets|decrypt-secret|ca init|issue-operator|upsert-cluster-authz-policy|get-cluster-authz-policy}` — same admin listener, dispatched to the `reflw.config.v1.Config` service (app config: deployments, secrets, cluster authz policy). **`ca init` and `issue-operator` are LOCAL commands** (no RPC, no cluster connection): the cluster CA is config + KMS — a public cert distributed via `cluster_ca.ca_cert_file` plus a KMS-wrapped signing key at `cluster_ca.key_blob_uri` (`+ key_kek_uri`), sealed/unwrapped with the fixed AAD `reflw.ClusterCAKeyAAD`. `ca init` mints the CA and seals its key locally; `issue-operator` mints an `operator/<name>` leaf from it locally — whoever can unwrap the KMS key can mint a cert. Each node **self-issues** its own `node/<id>` mesh leaf at startup (certmgr.BuiltinIssuer + CertMagic), so there is no central issuer, no join token, and no bootstrap port — the `MeshSign` service, `internal/bootstrap`, shard-0 `CARootTable`/`JoinTokenTable`, and `certmgr.ClusterIssuer` were all removed. DeploymentTable carries a CAS revision (`RevisionTableDeployment`); `delete-deployment` requires `--force` because in-flight invocations resolve their pinned deployment per stream-open and a delete can break them.
...
- **Tenancy is a deployment concern, not an engine concept.** The engine is single-tenant — there is no tenant band in `partition_key`, no `tenant/<n>` principal, no per-tenant key prefix; multi-tenancy is one engine instance per customer. `keys.LPCount` = 4096 Logical Partitions; routing reduces `partition_key % LPCount` to an LP id (the `<lp:4>` prefix on every LP-keyed row), and LP→shard ownership lives in `LPOwnersTable` (`LPFromPartitionKey` → `LPOwnersTable`). Construct a key via `routing.PartitionKey(svc, key)`.
...
- **Cluster-managed config is on shard 0.** Deployments, secrets, and the cluster authz policy live in shard 0's typed tables (`DeploymentTable`, `SecretTable`, `PlatformConfigTable`). Mutations propose `Command_Upsert*/Delete*` with `Envelope.precondition.if_table_revision_eq` for CAS; the FSM signals CAS failure via `Result.Value = ResultValueFailedPrecondition` (returning an error halts the shard, per `internal/engine/CLAUDE.md`). Per-node subsystems subscribe to a `cluster.TableNotifier` and pull a fresh snapshot on wake (5s ticker backstop). **Secrets do not traverse Raft**: `SecretRecord` carries pointer fields (`remote_encrypted { blob_uri, kek_uri }`) only; per-node `internal/secretstore` Resolvers fetch via `gocloud.dev/blob`, dispatch KEK through Tink's `KMSClient` registry, and decrypt with `AAD = []byte(secret.name)` — the reusable primitive is `secretstore.ResolveRemoteEncrypted`. The cluster CA is **not** a shard-0 table: it is config + KMS (public cert in `cluster_ca.ca_cert_file`, KMS-wrapped key at `cluster_ca.key_blob_uri`), resolved per-node at startup via `ResolveRemoteEncrypted` so a joiner can self-issue its mesh leaf before joining the cluster. Four KMS providers ship in-binary at `pkg/kms/{blob,awskms,gcpkms,hcvault}/` (always-linked, config-gated).
- **Arbitrary dedup is LP-keyed; self dedup is shard-scoped.** `dedup/arbitrary/<lp:4>/<producer>/<seq>` rows ride the LP-transfer scan so a producer retry that hits the LP's new owner after a transfer flip finds its dedup row already present. The LP is derived from the command kind via `partition.lpFromCommand`; LP-agnostic kinds (today only `OutboxAck`) key under `keys.LPNoLP = 0xFFFF_FFFF`, a sentinel that is never a real LP (real LPs are < `keys.LPCount` = 4096) and is therefore never range-deleted by `FinishLPTransfer`. `dedup/self/*` stays shard-scoped per leader epoch (`GCSelfBelowEpoch` reclaims old epochs on leader gain).
