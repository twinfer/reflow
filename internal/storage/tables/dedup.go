...
// are LP-prefixed so they ride the LP-transfer scan and follow the LP across
// shard moves. SelfProposal dedup is shard-scoped to leader_epoch and ignores
// lp.
...
// namespace and the sequence number so each propose has its own slot. lp is
// consumed only for the Arbitrary variant. The boolean is false for an empty
// Dedup (kind unset).
