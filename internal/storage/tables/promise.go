...
// (nil, nil) when absent.
func (t PromiseTable) Get(service, workflowKey, name string) (*enginev1.PromiseValue, error) {
	var v enginev1.PromiseValue
	if err := getProto(t.S, keys.PromiseKey(service, workflowKey, name), &v); err != nil {
...
func (t PromiseTable) Put(b storage.Batch, service, workflowKey, name string, v *enginev1.PromiseValue) error {
	return putProto(b, keys.PromiseKey(service, workflowKey, name), v)
}

// Delete removes the PromiseValue row. Used by the workflow retention reaper.
func (t PromiseTable) Delete(b storage.Batch, service, workflowKey, name string) error {
	return b.Delete(keys.PromiseKey(service, workflowKey, name))
}

// DeleteAllForWorkflow range-deletes every promise row under
// (service, workflow_key). Used by the workflow retention reaper.
func (t PromiseTable) DeleteAllForWorkflow(b storage.Batch, service, workflowKey string) error {
	prefix := keys.PromisePrefixForWorkflow(service, workflowKey)
