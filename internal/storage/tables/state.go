...
func (t StateTable) Get(lp uint32, target *enginev1.InvocationTarget, key string) ([]byte, bool, error) {
	k := keys.StateKey(lp, target.GetServiceName(), target.GetObjectKey(), key)
...
func (t StateTable) Set(b storage.Batch, lp uint32, target *enginev1.InvocationTarget, key string, value []byte) error {
	k := keys.StateKey(lp, target.GetServiceName(), target.GetObjectKey(), key)
	return b.Set(k, value)
}

// Clear deletes a single (target, key) row.
func (t StateTable) Clear(b storage.Batch, lp uint32, target *enginev1.InvocationTarget, key string) error {
	k := keys.StateKey(lp, target.GetServiceName(), target.GetObjectKey(), key)
	return b.Delete(k)
}
...
func (t StateTable) ClearObject(b storage.Batch, lp uint32, target *enginev1.InvocationTarget) error {
	prefix := keys.StatePrefixForObject(lp, target.GetServiceName(), target.GetObjectKey())
	...
	if upper == nil {
		...
		return t.ScanObject(lp, target, func(key string, _ []byte) error {
			return t.Clear(b, lp, target, key)
		})
	}
...
func (t StateTable) ScanObject(lp uint32, target *enginev1.InvocationTarget, fn func(key string, value []byte) error) error {
	prefix := keys.StatePrefixForObject(lp, target.GetServiceName(), target.GetObjectKey())
	upper := keys.PrefixUpperBound(prefix)
