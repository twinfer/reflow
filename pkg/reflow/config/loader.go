// Package config builds reflow.Config from layered sources via koanf.
//
...
// FromEnvPrefix when embedding reflow into a binary with its own
// namespace.
const DefaultEnvPrefix = "REFLOW_"
...
// Load merges sources into a reflow.Config. Later sources override
...
// DataDir non-empty) happens in reflow.Run, not here — the loader is
// composable and a partial config is a legal intermediate state.
func Load(sources ...Source) (reflow.Config, *koanf.Koanf, error) {
	k := koanf.New(".")
	for i, src := range sources {
		if src.Provider == nil {
			return reflow.Config{}, nil, fmt.Errorf("config: source %d: nil provider", i)
		}
		if err := k.Load(src.Provider, src.Parser); err != nil {
			return reflow.Config{}, nil, fmt.Errorf("config: source %d load: %w", i, err)
		}
	}
	var cfg reflow.Config
	if err := k.Unmarshal("", &cfg); err != nil {
		return reflow.Config{}, nil, fmt.Errorf("config: unmarshal: %w", err)
	}
	return cfg, k, nil
}
...
// Mapping: REFLOW_<SECTION>_<snake_case_field> → section.snake_case_field.
// One underscore separates section from field; underscores inside the
// field name are preserved. Examples:
//
//	REFLOW_NODE_ID                 → node.id
//	REFLOW_NODE_RAFT_ADDR          → node.raft_addr
//	REFLOW_STORAGE_DATA_DIR        → storage.data_dir
