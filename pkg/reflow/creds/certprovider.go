...
func buildCertProvider(s *CertProviderSpec, _ *slog.Logger) (*ListenerCreds, error) {
	...
	if s.Roots != "" {
		...
		if perr != nil {
			identity.Close()
			return nil, fmt.Errorf("reflow/creds: parse roots config: %w", perr)
		}
		roots, err = rootsCfg.Build(certprovider.BuildOptions{})
		if err != nil {
			identity.Close()
			return nil, fmt.Errorf("reflow/creds: build roots provider: %w", err)
		}
	}
	...
	getCert := func(_ *tls.ClientHelloInfo) (*tls.Certificate, error) {
		...
		if len(km.Certs) == 0 {
			return nil, errors.New("reflow/creds: identity provider returned no certs")
		}
	...
	verifyRoots := func() (*tls.Config, *tls.Config, error) {
		...
		if roots != nil {
			...
			if kerr != nil {
				return nil, nil, fmt.Errorf("reflow/creds: roots KeyMaterial: %w", kerr)
			}
			if s.clientAuth() {
