// Package reflw is the public entrypoint for embedding the reflw durable
// execution engine in a Go binary. Construct a Config (programmatically or
// via pkg/reflw/config loaders), declare handler deployments via
...
// Config is the typed configuration for a reflw node. All fields are
...
// translates REFLW_INGRESS_GRPC_ADDR → ingress.grpc_addr).
//
...
// ProcessConfig enables BPMN/CMMN process execution via the reflwos engine
// adapter (pkg/reflw/processengine). Like Handlers.InProcess it is set
...
type ProcessConfig struct {
	...
	// (operators register models via `reflwd config register-model`). Settable
	...
	// Models resolves pinned reflwos model refs (kind+name+version) to parsed
	...
	Models processengine.ModelResolver `koanf:"-"`
...
// handler-deployment URLs reflw.Run auto-registers at metadata-leader
...
//
//	file:///mnt/reflw-snaps       local filesystem (dev / NFS / shared volume)
//	s3://bucket?prefix=reflw/     AWS S3
//	gs://bucket?prefix=reflw/     Google Cloud Storage
...
// endpoints) ride on the URL query string; reflw doesn't interpret
...
type SnapshotConfig struct {
	...
	// archiving. Empty falls back to $TMPDIR/reflw-snapshot-scratch.
...
//     path the manual `reflwd cluster transfer-lp` CLI uses,
...
type NodeConfig struct {
	...
	// DeliveryAddr is the host:port for this node's reflw Delivery gRPC
...
type Peer struct {
	...
	// AdminAddr, when set, is this peer's reflw Admin gRPC endpoint.
	// Not consulted by the joiner SelfJoin path (which discovers the
	// leader via gossip-published NodeHostMeta.admin_endpoint), but
	// kept here so the `reflwd cluster ...` CLI and tests can resolve peers
...
type ClusterConfig struct {
	...
	// run `reflwd cluster add-node` (which proposes RegisterNode +
...
type StorageConfig struct {
	...
	// replaces Pebble's per-DB default, which on reflw's
...
// content negotiation. reflw.Run starts ingress unless Disabled is
