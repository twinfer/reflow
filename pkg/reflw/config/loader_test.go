...
`
	...
	if cfg.Storage.DataDir != "/tmp/reflw" {
...
func TestLoad_EnvOverridesFile(t *testing.T) {
	...
	t.Setenv("REFLW_NODE_RAFT_ADDR", "env:2")
	t.Setenv("REFLW_STORAGE_DATA_DIR", "envdata")
...
func TestLoad_FromEnvOnly(t *testing.T) {
	t.Setenv("REFLW_NODE_ID", "9")
	t.Setenv("REFLW_NODE_RAFT_ADDR", ":5410")
	t.Setenv("REFLW_STORAGE_DATA_DIR", "/var/reflw")
	t.Setenv("REFLW_INGRESS_ADDR", ":8080")
	t.Setenv("REFLW_LOGGING_LEVEL", "DEBUG")
	...
	if cfg.Storage.DataDir != "/var/reflw" {
		t.Errorf("Storage.DataDir = %q", cfg.Storage.DataDir)
	}
	if cfg.Ingress.Addr != ":8080" {
...
func TestLoad_SecretsAsConfig(t *testing.T) {
	cfg, _, err := config.Load(
		config.FromMap(map[string]any{
			...
			"ingress.creds.tls.cert_file": "/etc/reflw/ingress.crt",
			"ingress.creds.tls.key_file":  "/etc/reflw/ingress.key",
			"ingress.creds.tls.ca_file":   "/etc/reflw/ca.crt",
		}),
	)
	if err != nil {
		t.Fatalf("Load: %v", err)
	...
	if cfg.Ingress.Creds.TLS.CertFile != "/etc/reflw/ingress.crt" {
