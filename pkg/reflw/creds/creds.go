...
// reflw. Each listener (Delivery, Admin, Ingress) and each handler-dial
...
// default is load-bearing for "go run ./cmd/reflwd" to start cert-free.
// Multi-node + insecure is allowed but emits a loud WARN at startup
// (see pkg/reflw/run.go); this package does not enforce the policy.
package creds
...
type ListenerCreds struct {
	...
	// (pkg/reflwclient, pkg/ingressclient, internal/engine/delivery
...
func Build(s Spec, log *slog.Logger) (*ListenerCreds, error) {
	...
	default:
		return nil, fmt.Errorf("reflw/creds: unknown driver %q", driver)
	}
}
...
func errMissingSpec(d Driver) error {
	return fmt.Errorf("reflw/creds: driver %q selected but spec is nil", d)
}

// errEmptyField is the common error for required string fields.
func errEmptyField(d Driver, field string) error {
	return fmt.Errorf("reflw/creds: driver %q: %s is required", d, field)
}
...
var ErrPerRPCRequiresTransport = errors.New(
	"reflw/creds: oauth/jwt/sts drivers require an underlying transport — wrap with creds.WithTransport or configure tls/tls_certprovider on the listener")
