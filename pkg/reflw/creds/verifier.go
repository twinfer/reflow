...
func NewVerifier(rootsPEM []byte, allowedPrincipals []string, audience string) (*Verifier, error) {
	if len(rootsPEM) == 0 {
		return nil, errors.New("reflw/creds: verifier requires non-empty rootsPEM")
	}
	if len(allowedPrincipals) == 0 {
		return nil, errors.New("reflw/creds: verifier requires at least one allowed principal")
	}
	pool := x509.NewCertPool()
	if !pool.AppendCertsFromPEM(rootsPEM) {
		return nil, errors.New("reflw/creds: verifier: rootsPEM contains no parseable PEM blocks")
	}
...
func (v *Verifier) Verify(bearer string) (*Verified, error) {
	if v == nil {
		return nil, errors.New("reflw/creds: nil Verifier")
	}
	if bearer == "" {
		return nil, errors.New("reflw/creds: empty bearer token")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("reflw/creds: verify: %w", err)
	}
	if !parsed.Valid {
		return nil, errors.New("reflw/creds: token invalid after parse")
	}
	if v.audience != "" && !audienceContains(claims.Audience, v.audience) {
		return nil, fmt.Errorf("reflw/creds: aud %v does not contain %q", []string(claims.Audience), v.audience)
	}
	aud := ""
