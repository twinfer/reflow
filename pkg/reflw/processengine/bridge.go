...
// The capability-bridge is one deployed reflw handler that services every
// TaskInvoke the adapter emits by running the named reflwos capability. These
...
const (
	// BridgeService is the reflw service name the bridge handler is deployed under.
	BridgeService = "reflwos.capability"
...
// bridge handler reads from ctx.Input(). It carries the reflwos capability ref plus
...
// byte-stable under replay. No tenant: reflw is single-tenant (reflwos's tenancy
...
// capability fault. reflw's terminal-failure code channel is a uint32 and the
...
// bridgeFault carries a coded BPMN business fault across reflw's terminal-
...
// code, reflw ferries the bytes verbatim (InvocationCompleted.failure_message →
// ProcessTaskCompleted.failure_message), and eventForBPMN re-splits them into
// bpmn.TaskFailed.{ErrorCode,Cause}. The envelope is reflwos-private — internal/
...
// envelope — every non-reflwos failure, every reflwos fault without a BPMN error
// code, and a reflw-synthesized failure (step-budget, cancel) — returns code ""
...
// bridgeHandler runs the reflwos capability named by the BridgeInput and returns
...
func bridgeHandler(capReg *capability.Registry) handler.Handler {
	return func(ctx handler.Context, input []byte) ([]byte, error) {
		...
		if err := json.Unmarshal(input, &bi); err != nil {
			return nil, handler.NewFailure(bridgeFailureCode, fmt.Sprintf("reflwos bridge: decode input: %v", err))
		}
...
// error-boundary logic runs and reflw does not retry; a bare error stays
// transient so reflw retries the side effect under the Run's backoff policy.
func runCapability(ctx context.Context, capReg *capability.Registry, bi BridgeInput) ([]byte, error) {
	h, ok := capReg.Resolve(bi.Ref)
	if !ok {
		return nil, handler.NewFailure(bridgeFailureCode, fmt.Sprintf("reflwos bridge: unresolved capability %q", bi.Ref))
	}
