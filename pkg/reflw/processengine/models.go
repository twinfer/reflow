// Package processengine adapts reflwos's pure BPMN/CMMN engines to reflw's
...
// reflwos engine turn, and translates the emitted Commands into a reflw
// ProcessAdvanced. reflw's partition actuates the instructions and feeds
// results back as the next turn.
//
// internal/engine never imports reflwos; the binding lives here and is injected
...
// the requested ModelRef. Advance surfaces it as an error, which reflw's
// processSession converts into a failed ProcessTerminal.
var ErrModelNotFound = errors.New("processengine: model not found")

// ModelResolver hands the adapter the parsed reflwos model for a ModelRef. It MUST
...
// production resolver would back this with a reflw modelstore plus a pre-warm
...
func (r *MapResolver) ParseBPMN(name, version string, xml []byte) error {
	...
	if err != nil {
		return fmt.Errorf("processengine: parse bpmn %q/%q: %w", name, version, err)
	}
...
func (r *MapResolver) ParseCMMN(name, version string, xml []byte) error {
	...
	if err != nil {
		return fmt.Errorf("processengine: parse cmmn %q/%q: %w", name, version, err)
	}
...
func decisionResolver(decisions map[string]*dmn.Runtime) bpmn.DecisionResolver {
	return func(ref string) (*dmn.Runtime, string, error) {
		rt, ok := decisions[ref]
		if !ok {
			return nil, "", fmt.Errorf("processengine: decision %q not registered", ref)
		}
		return rt, ref, nil
	}
}
