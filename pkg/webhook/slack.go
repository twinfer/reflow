...
// slackVerifier implements Verifier for Slack v0 signatures. Signed
// string: "v0:" + X-Slack-Request-Timestamp + ":" + body. HMAC-SHA256
// keyed on the signing secret, lowercase hex, header format:
...
func NewSlackVerifier(tolerance time.Duration) Verifier {
	if tolerance <= 0 {
...
func init() { RegisterVerifier(NewSlackVerifier(0)) }

func (s *slackVerifier) Name() string { return "slack" }

func (s *slackVerifier) Verify(_ context.Context, r *http.Request, secret []byte) (*VerifiedEvent, error) {
	if len(secret) == 0 {
	...
	return &VerifiedEvent{
		...
		Metadata: map[string]string{
			MetadataKeyVendor:        "slack",
			"slack_signed_timestamp": tsHeader,
		},
		// Slack Events API bodies carry "event_id"; slash-command bodies
		// are form-encoded and yield "" (best-effort).
		IdempotencyKey: jsonStringField(body, "event_id"),
	}, nil
}
