...
// TestSlackVerifier_EventIDIdempotencyKey confirms an Events-API JSON
// body surfaces "event_id" as the idempotency key (slash commands are
// form-encoded and yield "", which the ValidSignature test covers).
func TestSlackVerifier_EventIDIdempotencyKey(t *testing.T) {
	f := defaultSlackFixture()
	f.body = []byte(`{"type":"event_callback","event_id":"Ev123","event":{"type":"message"}}`)
	r := f.request(t, f.sign(t), strconv.FormatInt(f.tsUnix, 10))
	ev, err := newTestSlackVerifier(f.now).Verify(context.Background(), r, f.secret)
	if err != nil {
		t.Fatalf("Verify: %v", err)
	}
	if ev.IdempotencyKey != "Ev123" {
		t.Errorf("IdempotencyKey=%q; want Ev123 (Slack event_id)", ev.IdempotencyKey)
	}
}
